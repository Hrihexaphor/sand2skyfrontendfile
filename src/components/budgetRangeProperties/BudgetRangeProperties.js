...
const BudgetRangeProperties = () => {
  ...
  const priceCategories = {
    "Properties Below ₹1 Cr": [0, 10000000],
    "Properties Between ₹1 Cr – ₹2 Cr": [10000000, 20000000],
    "Properties Between ₹2 Cr – ₹4 Cr": [20000000, 40000000],
    "Properties Above ₹4 Cr": [40000000, Infinity],
  };
  ...
  useEffect(() => {
    if (!title || !properties.length) return;

    const range = priceCategories[title];
    if (!range) return;
...
console.log(filtered)
  ...
  const parseBudget = (val) => {
    ...
    return val.includes('CR') ? num * 1e7 : num;
  };
...
export default BudgetRangeProperties;
